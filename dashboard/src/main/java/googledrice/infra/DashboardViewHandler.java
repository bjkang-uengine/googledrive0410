...
public class DashboardViewHandler {
    ...
    public void whenFileUploaded_then_CREATE_1(
        ...
        try {
            ...
            dashboard.setFileName(Long.valueOf(fileUploaded.getName()));
            dashboard.setIdFileUpoaded(true);
            // view 레파지 토리에 save
    ...
    public void whenIndexed_then_UPDATE_1(@Payload Indexed indexed) {
        try {
            ...
            for (Dashboard dashboard : dashboardList) {
                ...
                dashboard.setIdFileIndexed(true);
                // view 레파지 토리에 save
                dashboardRepository.save(dashboard);
            }
        } catch (Exception e) {
    ...
    public void whenVideoProcessed_then_UPDATE_2(
        ...
        try {
            ...
            for (Dashboard dashboard : dashboardList) {
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                dashboard.setVideoUrl(videoProcessed.getViderUrl());
                // view 레파지 토리에 save
                dashboardRepository.save(dashboard);
            }
        } catch (Exception e) {
